'use strict';
// Express の Router モジュールをテストする
const request = require('supertest');
const assert = require('assert');
// テストの対象の読み込み
const app = require('../app');
const passportStb = require('passport-stub');
...
describe('/login', () => {
  // before 関数で記述された処理は describe 内のテスト前に、
  // after 関数で記述された処理は describe 内のテストの後に実行
  // mochaの機能
  before(() => {
    passportStb.install(app);
    passportStb.login({
      username: 'testuser'
    });
  });
  after(() => {
    passportStb.logout();
    passportStb.uninstall(app);
  });

  it('ログインのためのリンクが含まれる', (done) => {
    // /login への GET リクエストを作成
    // テストを終了する際には、 expect 関数に、期待されるステータスコードの整数と
    // テスト自体の引数に渡される done 関数を渡す
...
describe('/schedules/:scheduleId?delete=1', (done) => {
  before(() => {
    passportStb.install(app);
    passportStb.login({
      id: 0,
